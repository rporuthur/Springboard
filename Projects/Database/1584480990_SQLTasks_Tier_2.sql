...
SELECT DISTINCT f.name, concat(m.firstname, ' ', m.surname) AS member_name, (f.membercost*b.slots) as cost
FROM Bookings b 
	INNER JOIN Facilities f 
	ON b.facid = f.facid
	INNER JOIN Members m
...
UNION
SELECT DISTINCT f.name, concat(m.firstname, ' ', m.surname) AS member_name, (f.guestcost*b.slots) as cost
FROM Bookings b 
	INNER JOIN Facilities f 
	ON b.facid = f.facid
	INNER JOIN Members m
...
FROM (SELECT distinct f.name, concat(m.firstname, ' ', m.surname) AS member_name, 
		...
		FROM Bookings b 
			...
			INNER JOIN Members m
				ON b.memid = m.memid
		WHERE b.starttime LIKE '2012-09-14%') a
WHERE cost> 30.0 
ORDER BY cost DESC;
